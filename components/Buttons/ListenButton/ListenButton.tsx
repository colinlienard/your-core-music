...
interface Props {
    url: string,
    text: string,
    getSmallOnMobile?: boolean
}

const ListenButton: FC<Props> = ({ url, text, getSmallOnMobile }) => {
    return (
        <a className={styles.ListenButton} href={url} target="_blank" rel="noreferrer">
            <svg width="19" height="20" viewBox="0 0 19 20" role="img" aria-label={text}>
            ...
            <p className={`${styles.text} ${getSmallOnMobile ? styles.small : null}`}>{text}</p>
        </a>
    )
}
